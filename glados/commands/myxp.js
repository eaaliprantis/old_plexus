...
module.exports = {
    get_xp: function (msg, bot, cmdexec) {
        ...
        if(args.length == 0) {
            ...
            if(!commands[user3]) {
                ...
                bot.createMessage(msg.channel.id, "You are level `" + Math.floor(commands[user3]['stats']["level"]) + "` with `" + Math.floor(commands[user3]['stats']["xp"]) + "` xp!");
            }
            delete require.cache[commands];

        } else {
            bot.createMessage(msg.channel.id, "Usage: " + cmdexec + "res");
        }
    }
};
